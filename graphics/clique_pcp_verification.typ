...
#let y_transcripts = verifier_mid_y
...
#let color_pi_stroke = black
#let color_arrow = black
#let color_accept = green
#let color_reject = red
  ...
  for j in (0, 1, 2) {
    only(1 + 4 * j)[
      #canvas(length: canvas_length, {
        ...
        // Scenari accettanti
        content((x_transcripts, y_transcripts), transcripts_text(j))
        ...
        content((verifier_mid_x, x_mid_y), text(size: 20pt)[$G$])
